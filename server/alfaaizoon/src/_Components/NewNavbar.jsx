...
export default function NewNavbar() {
  ...
  useEffect(() => {
    ...
    setrole(userToken ? JSON.parse(userToken).role : "");
    if (token || userToken) {
      setSucessLogin(true); // User is logged in if any token exists
    } else {
      setSucessLogin(false); // No token means user is not logged in
    }
  }, []);
